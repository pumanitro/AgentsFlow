  ...
  return <div className="h-full flex flex-col min-h-0 bg-bg text-text">
    ...
    <div ref={scroll} onScroll={() => { const el = scroll.current; if (el) follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100; }} className="flex-1 min-h-0 overflow-auto p-5 select-text">
      <div className="max-w-4xl mx-auto">
        ...
        {snapshot?.entries.map((entry) => entry.id === 'handover' ? <p key={entry.id} className="mb-4 text-xs text-muted italic border-l-2 border-border pl-3 whitespace-pre-wrap break-words">{entry.text}</p>
          : entry.role === 'tool' ? <details key={entry.id} className="mb-2 text-xs rounded border border-border bg-panel p-2"><summary className="cursor-pointer truncate text-muted">{entry.status ? `${stateLabel(entry.status)} · ` : ''}{entry.text.split('\n')[0]}</summary><pre className="mt-2 whitespace-pre-wrap break-words max-h-80 overflow-auto">{entry.text}</pre></details>
